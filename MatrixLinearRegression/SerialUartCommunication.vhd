...
PACKAGE uart2_pkg IS
	...
	END COMPONENT;
END PACKAGE uart2_pkg;

LIBRARY ieee;
USE ieee.std_logic_1164.all;
use work.uart2_pkg.all;

ENTITY SerialUartCommunication IS
	GENERIC(
		clk_freq		:	INTEGER		:= 50_000_000;	--frequency of system clock in Hertz
		baud_rate	:	INTEGER		:= 115_200;		--data link baud rate in bits/second
		os_rate		:	INTEGER		:= 16;			--oversampling rate to find center of receive bits (in samples per baud period)
...
ARCHITECTURE logic OF SerialUartCommunication IS
	TYPE 		tx_machine IS(idle, transmit);										--tranmit state machine data type
	TYPE 		rx_machine IS(idle, receive, ready_st);											--receive state machine data type
	...
	SIGNAL	parity_error, rx_ready_aux		:	STD_LOGIC;										--receive parity error flag
...
BEGIN
	...
	BEGIN
		IF(reset_n = '0') THEN																--asynchronous reset asserted
			...
			rx_error <= '0';
			rx_ready_aux <= '0';																		--clear receive errors
		...
		ELSIF(clk'EVENT AND clk = '1' AND os_pulse = '1') THEN  					--enable clock at oversampling rate
			CASE rx_state IS
				WHEN idle =>																		--idle state
					rx_busy <= '0';
					rx_ready_aux <= '0';																		--clear receive busy flag
				...
				WHEN receive =>																	--receive state
					...
					ELSE																					--center of stop bit
						...
						rx_busy <= '0';	
						rx_ready_aux <= '1';																--deassert received busy flag
						rx_state <= ready_st;																	--return to idle state
					END IF;
				WHEN ready_st =>
				        rx_ready_aux <= '0';															--deassert received busy flag
                        rx_state <= idle;    
			END CASE;
        END IF;
	END PROCESS;
	...
	os_pulse_out <= os_pulse;
	rx_ready <= rx_ready_aux;
